...
begin
  ...
  if (i > 1) or (n > 1) then begin
    with dm.qryTargetData do begin // get user_nr from user_t
      Close;
...
//      ClipBoard.SetTextBuf(PChar(s));
      ...
      if RecordCount > 0 then begin
        j := RecordCount;
        StrGrdTargetData.Clear;
        with StrGrdTargetData {, grdTargetData} do begin
          ColCount := 8;
          RowCount := j + 1;
...
begin
  ...
  begin
    ...
    for i := 0 to grdMagazineData.FieldCount - 1 do begin
...
begin
  ...
  with dm.qryProjectsOfUser do begin     //get all projects of the selected user
    if chkAllProjects.Checked then begin
      ...
      if Length(cmbProjectOfReport.Text) > 0 then begin
        ...
        if dm.qryProjectsOfUser.RecordCount > 0 then begin
  ...
  if dm.qryProjectsOfUser.RecordCount > 0 then begin
    with dm.qrySampleOfSubmitter do begin
      ...
      if not DoExportHalle then begin
        ...
        if radgrpStatus.ItemIndex < 3 then begin
...
//     Kollagengehalt (%)	C:N ratio	F14C	Magazin
 ...
 //     ClipBoard.SetTextBuf(PChar(s));
    ...
    with grdSamplesOfSubmitter do begin
      Columns[0].Width := 55;
      //Columns[1].Width := 55;
      Columns[1].Width := 140;
      Columns[2].Width := 100;
...
begin
  wizInputSamples.ActivePage := wizStartPage;
  with edtNewSamplesFilename do begin
    if Length(Filename) > 0 then begin // strip previously used filename
...
procedure TfrmMAMS.SetupOptions;
var
  i: integer;
begin
...
procedure TfrmMAMS.wizFinalPageFinishButtonClick(Sender: TObject;
...
begin
  ...
  with grdShowPrepSteps do begin
    RowHeights[8] := 5;
  end;
