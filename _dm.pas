...
//    qryAllUser: TADOQuery;
  ...
  public
    ...
    function GetProjectNrByProject(project_name: string): string;
...
begin
 ...
 (*
      IF InsertIntoDB('project_t', 'project', project_name) THEN BEGIN
        ...
        ShowMessage('Error inserting new project.');
        Result:='none'
      END
      ELSE begin
        // data inserted without any error
        ShowMessage('New project created.');  // show message
        Result:= dm.GetProjectNrByProject(project_name);// return project_nr of the newly created project
      end;
    END;
 *)
end;
...
begin
 with qryMagazineData do begin
    SQL.Text := 'SELECT position, project_t.project, user_t.last_name, sample_t.sample_nr, ' +
                ' target_t.prep_nr, target_t.target_nr, target_t.c14_age  ' +
                ' FROM target_t ' +
...
function Tdm.GetProjectNrByProject(project_name: string): string;
var
  s : string;
begin
   with qryDb do begin
     Close;
     SQL.Text := 'SELECT project_nr FROM project_t WHERE project='+ project_name + ';';
     s := SQL.Text;
     //ClipBoard.SetTextBuf(PChar(s));
     Open;
     Result := Fields.Fields[0].AsString;
