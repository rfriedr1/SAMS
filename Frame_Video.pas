...
// this is a frame that can be reused in the same form many times
// it is being used in the form: FormCamera.pas
...
procedure TVideoFrame.UpdateCamList;
var
  SL : TStringList;
begin
...
procedure TVideoFrame.CalcInvertedImage(BM1, Inv: TBitmap);
VAR
  X, Y  : integer;
  p1, d : pbytearray;
begin
...
procedure TVideoFrame.CalcGrayScaleImage(BM1, Gray: TBitmap);
VAR
  X, Y, i : integer;
  p1, d   : pbytearray;
  g       : byte;
...
procedure TVideoFrame.CalcDiffImage(BM1, BM2, Diff: TBitmap; VAR DiffRatio: double);
VAR
  X, Y      : integer;
  p1, p2, d : pbytearray;
  TotalDiff : integer;
...
procedure TVideoFrame.CalcDiffImage2(BM1, BM2, Diff: TBitmap; VAR DiffRatio: double);
begin
...
procedure TVideoFrame.OnNewFrame(Sender : TObject; Width, Height: integer; DataPtr: pointer);
VAR
  i, x, y,
  T1 : integer;
  d  : double;
...
procedure TVideoFrame.InitFrame;
// initializes the buttons etc of the frame
// also initializes the video object
var
  i  : integer;
  VP : TVideoProperty;
begin
...
procedure TVideoFrame.CleanPaintBoxVideo;
begin
...
procedure TVideoFrame.Updatelistofcameras1Click(Sender: TObject);
begin
  UpdateCamList;
end;

procedure TVideoFrame.ComboBox1Change(Sender: TObject);
begin
...
procedure TVideoFrame.SpeedButton_RunVideoClick(Sender: TObject);
{ - Start live video }
var
  i         : integer;
  SL        : TStringList;
...
procedure TVideoFrame.SpeedButton_PauseClick(Sender: TObject);
begin
  IF assigned(VideoImage) then
...
procedure TVideoFrame.SpeedButton_StopClick(Sender: TObject);
begin
  SpeedButton_VidSettings.Enabled := false;
  SpeedButton_VidSize.Enabled := false;
  Screen.Cursor := crHourGlass;
...
procedure TVideoFrame.SpeedButton_VidSettingsClick(Sender: TObject);
begin
  VideoImage.ShowProperty;
  CleanPaintBoxVideo;
end;

procedure TVideoFrame.SpeedButton_VidSizeClick(Sender: TObject);
begin
  IF assigned(VideoImage) then
    begin
      CleanPaintBoxVideo;
...
procedure TVideoFrame.PropertyTrackBarChange(Sender: TObject);
VAR
  VP : TVideoProperty;
begin
...
procedure TVideoFrame.PropertyCheckBoxClick(Sender: TObject);
VAR
  VP : TVideoProperty;
begin
...
procedure TVideoFrame.SpeedButton1Click(Sender: TObject);
begin
